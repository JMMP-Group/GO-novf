...
def FindSec(ds_domcfg, target_lons, target_lats):
    '''
    This function uses the Bresenham alogorithm to identify 
    all the T points along a user defined section and then 
    extracts all the U, V, or F points in between those 
    T-points.
    '''
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 2. Find the closest T-points to the user-defined points
    ds_tgt = nrst_nghbr(target_lons, target_lats, grids, 't')
    pnts_x_t, pnts_y_t = (ds_tgt[f"{i}_index"].data for i in ("x", "y"))

    # ---------------------------------------------------------
    # 3. Use the Bresenham alogorithm to identify all the T points 
    #    along a user defined section
    ...
    # ---------------------------------------------------------
    # 4. Extract all the U, V, or F points in between those T-points
...
def extract_section_from_array(ds_domcfg, ds_scalar, vvar, target_lons, target_lats):
    ...
    if fpnt:
       var3D_F = ds_scalar[vvar].rolling(x=2).mean().shift(x=-1).fillna(-9999.)
       var3D_F = var3D_F.rolling(y=2).mean().shift(y=-1).fillna(-9999.)
       var2D_F = var3D_F.isel(x=sec_pnts['f'].x_index,
                              y=sec_pnts['f'].y_index
                             )
    else:
       # DataArray with Nans
       var2D_F = xr.DataArray(coords=var2D_T.coords,
                              dims=var2D_T.dims
                             )
    ...
    #print('var2D_F' ,var2D_F)

    var2D_UVF = xr.concat([var2D_U, var2D_V, var2D_F], dim="points").dropna(dim='points')
    var2D_UVF = var2D_UVF.reindex({'points':sorted(var2D_UVF.points)})
    var2D_UVF = var2D_UVF.where(var2D_UVF!=-9999.)

    #print(var2D_UVF)

    var2D = combine_arrays_alternating_columns(var2D_T, var2D_UVF)
    ...
    if fpnt:
       gdepf  = ds_domcfg["gdepf_0"].isel(x=sec_pnts['f'].x_index,
                                          y=sec_pnts['f'].y_index
                                         )
       gdepfw = ds_domcfg["gdepfw_0"].isel(x=sec_pnts['f'].x_index,
                                           y=sec_pnts['f'].y_index
                                          )
    else:
       gdepf = xr.DataArray(coords=gdept.coords,
                            dims=gdept.dims
                           )
       gdepfw = xr.DataArray(coords=gdepw.coords,
                             dims=gdepw.dims
                            )

    gdepuvf = xr.concat([gdepu, gdepv, gdepf], dim="points").dropna(dim='points')
    gdepuvf = gdepuvf.reindex({'points':sorted(gdepuvf.points)})
    gdepuvfw = xr.concat([gdepuw, gdepvw, gdepfw], dim="points").dropna(dim='points')
    gdepuvfw = gdepuvfw.reindex({'points':sorted(gdepuvfw.points)})

    # Combining
    gdepcw = combine_arrays_alternating_columns(gdepw, gdepuvfw)
    gdepc  = combine_arrays_alternating_columns(gdept, gdepuvf)
    ...
    if fpnt:
       glamf = ds_domcfg["glamf"].isel(x=sec_pnts['f'].x_index,
                                       y=sec_pnts['f'].y_index
                                      )
       gphif = ds_domcfg["gphif"].isel(x=sec_pnts['f'].x_index,
                                       y=sec_pnts['f'].y_index
                                      )
    else:
       glamf = xr.DataArray(coords=glamt.coords,
                            dims=glamt.dims
                           )
       gphif = xr.DataArray(coords=gphit.coords,
                            dims=gphit.dims
                           )

    glamuvf = xr.concat([glamu, glamv, glamf], dim="points").dropna(dim='points')
    glamuvf = glamuvf.reindex({'points':sorted(glamuvf.points)})
    gphiuvf = xr.concat([gphiu, gphiv, gphif], dim="points").dropna(dim='points')
    gphiuvf = gphiuvf.reindex({'points':sorted(gphiuvf.points)})

    min_length = min(len(glamt), len(glamuvf))
    glamc = [val for pair in zip(glamt, glamuvf) for val in pair]
    glamc.extend(glamt[min_length:])
    glamc.extend(glamuvf[min_length:])

    gphic = [val for pair in zip(gphit, gphiuvf) for val in pair]
    gphic.extend(gphit[min_length:])
    gphic.extend(gphiuvf[min_length:])
    ...
    distc = distc.expand_dims({"z": gdepuvf.shape[0]})
    ...
    if "loc_msk" in ds_domcfg.variables:
       ...
       if fpnt:
          loc_msk_F = ds_domcfg["loc_msk"].isel(x=sec_pnts['f'].x_index,
                                                y=sec_pnts['f'].y_index
                                               )
       else:
          loc_msk_F = xr.DataArray(coords=loc_msk_T.coords,
                                   dims=loc_msk_T.dims
                                  )

       loc_msk_UVF = xr.concat([loc_msk_U, loc_msk_V, loc_msk_F], dim="points").dropna(dim='points')
       loc_msk_UVF = loc_msk_UVF.reindex({'points':sorted(loc_msk_UVF.points)})

       min_length = min(len(loc_msk_T), len(loc_msk_UVF))
       loc_msk = [val for pair in zip(loc_msk_T, loc_msk_UVF) for val in pair]
       loc_msk.extend(loc_msk_T[min_length:])
       loc_msk.extend(loc_msk_UVF[min_length:])
