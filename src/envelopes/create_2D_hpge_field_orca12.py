...
HPGEdir = '/scratch/dbruciaf/GOSI-NOVF/mes_u-dn555/12_r16-r075-r040-r035-r030-r025-r020-r015_itr3-r010'

# List of indexes of the last T-level of each vertical subdomains 
# (Fortran indexening convention)
# N.B: in theory, the last T level of the 3rd zone should be 60, but we include 2 levels
#      of the 4th zone to be sure we don't miss high spurious currents at the very bottom 
#      (although the last two levels should be land anyway ...)
#num_lev = [40,62]
num_lev = [74]
...
for F in range(len(Ufiles)):
    ...
    nI = ds_U[Uvar].shape[3]
    nJ = ds_U[Uvar].shape[2]
    ...
    if F == 0:
       ...
       if len(num_lev) > 1:
          ...
          HPGE2     = np.zeros(shape=(nJ,nI))
          HPGE3     = np.zeros(shape=(nJ,nI))
       else:
          HPGE1     = np.zeros(shape=(nJ,nI))
...
ds_hpge = xr.Dataset()
if len(num_lev) > 1:
   HPGE2[y1:y2,x1:x2] = max_hpge2[:,:]
   HPGE3[y1:y2,x1:x2] = max_hpge3[:,:]
   ds_hpge["max_hpge_2"] = xr.DataArray(HPGE2, dims=('y','x'))
   ds_hpge["max_hpge_3"] = xr.DataArray(HPGE3, dims=('y','x'))
else:
   HPGE1[y1:y2,x1:x2] = max_hpge1[:,:]
   ds_hpge["max_hpge_1"] = xr.DataArray(HPGE1, dims=('y','x'))
